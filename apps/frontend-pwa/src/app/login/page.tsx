...
export default function LoginPwaPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // "Failed to fetch" = auth-api inalcanzable. Mensaje útil al usuario.
      setError(
        msg === 'Failed to fetch'
  ...
  return (
    <main style={wrap}>
      ...
      <p style={subtitle}>Chofer / Operario</p>
      ...
      <p style={foot}>
        ¿No tenés cuenta? <Link href="/registro" style={link}>Registrate</Link>
      </p>
      <p style={hint}>
        Si trabajas como chofer también podés acceder directo a{' '}
        <Link href="/chofer/cobrar" style={link}>cobrar</Link> o{' '}
        <Link href="/chofer/escanear" style={link}>escanear</Link>.
      </p>
    </main>
  );
}
...
const title: React.CSSProperties = { fontSize: 24, fontWeight: 700, margin: '0 0 4px', color: '#1f2937' };
const subtitle: React.CSSProperties = { color: '#6b7280', margin: '0 0 16px', fontSize: 13 };
...
const hint: React.CSSProperties = { marginTop: 8, fontSize: 12, color: '#9ca3af' };
